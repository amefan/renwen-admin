...
<el-input v-model="searchMap.state" placeholder="1 可见，0不可见"></el-input></el-form-item>
  ...
  <el-dialog title="编辑" :visible.sync="dialogFormVisible">
    <el-form label-width="80px">
        ...
        <el-form-item label="1 可见，0不可见"><el-input v-model="pojo.state"></el-input></el-form-item>
...
export default {
  ...
  methods: {
    fetchData() {
      confessionApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
        this.list = response.data.rows
        this.total = response.data.total
      })
    },
    handleSave() {
      confessionApi.update(this.id, this.pojo).then(response => {
        this.$message({
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        confessionApi.findById(id).then(response => {
          if (response.flag) {
    ...
    handleDelete(id) {
      this.$confirm('确定要删除此纪录吗?', '提示', {
        ...
        confessionApi.deleteById(id).then(response => {
